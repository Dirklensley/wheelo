...
func SetupRoutes(host, clientId, clientSecret string, endpoints map[string]string) http.Handler {
	...
	r.HandleFunc("/photos", open.LoginMiddleware(v, Photos(tmpl))).Methods(http.MethodGet)
	r.HandleFunc("/personal", open.LoginMiddleware(v, Personal(tmpl))).Methods(http.MethodGet)
